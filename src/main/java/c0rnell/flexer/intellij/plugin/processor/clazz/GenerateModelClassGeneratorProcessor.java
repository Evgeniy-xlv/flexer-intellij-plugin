...
public class GenerateModelClassGeneratorProcessor extends AbstractClassProcessor {
    ...
    protected boolean validate(@NotNull PsiAnnotation psiAnnotation, @NotNull PsiClass psiClass, @NotNull ProblemBuilder builder) {
        ...
        if (PsiAnnotationUtil.getBooleanAnnotationValue(psiAnnotation, "lazy", false)) {
            builder.addWarning(FlexerBundle.message("inspection.message.lazy.not.supported.for.getter.on.type"));
        }
    ...
    private boolean validateAnnotationOnRightType(@NotNull PsiClass psiClass, @NotNull ProblemBuilder builder) {
        ...
        if (psiClass.isAnnotationType() || psiClass.isInterface()) {
            builder.addError(FlexerBundle.message("inspection.message.getter.only.supported.on.class.enum.or.field.type"));
            result = false;
        }
        return result;
    }
...
//        filterToleratedElements(classMethods);
        ...
        for (PsiField psiField : psiClass.getFields()) {
            ...
            if (null != modifierList) {
                ...
                createGetter &= !psiField.getName().startsWith(Utils.INTERN_FIELD_MARKER);
                //Skip fields if a method with same name and arguments count already exists
                final AccessorsInfo accessorsInfo = AccessorsInfo.build(psiField);
                final Collection<String> methodNames = Utils.toAllGetterNames(accessorsInfo, psiField.getName(), PsiType.BOOLEAN.equals(psiField.getType()));
