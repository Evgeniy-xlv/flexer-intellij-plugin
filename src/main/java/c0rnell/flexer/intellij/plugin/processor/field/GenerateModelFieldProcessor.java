...
import c0rnell.flexer.intellij.plugin.processor.FlexerPsiElementUsage;
...
public class GenerateModelFieldProcessor extends AbstractFieldProcessor {
    ...
    protected void generatePsiElements(@NotNull PsiField psiField,
        ...
        final String methodVisibility = ProcessorUtil.getMethodModifier(psiAnnotation);
...
//            target.add(createGetterMethod(psiField, psiClass, methodVisibility));
    ...
    protected boolean validate(@NotNull PsiAnnotation psiAnnotation, @NotNull PsiField psiField, @NotNull ProblemBuilder builder) {
        ...
        if (null == methodVisibility && lazy) {
            builder.addWarning(FlexerBundle.message("inspection.message.lazy.does.not.work.with.access.level.none"));
        }

        if (result && lazy) {
            if (!psiField.hasModifierProperty(PsiModifier.FINAL) || !psiField.hasModifierProperty(PsiModifier.PRIVATE)) {
                builder.addError(FlexerBundle.message("inspection.message.lazy.requires.field.to.be.private.final"),
            ...
            if (!psiField.hasInitializer()) {
                builder.addError(FlexerBundle.message("inspection.message.lazy.requires.field.initialization"));
                result = false;
            }
        }
    ...
    private boolean validateExistingMethods(@NotNull PsiField psiField, @NotNull ProblemBuilder builder) {
        ...
        if (null != psiClass) {
            ...
            final Collection<String> methodNames = Utils.toAllGetterNames(accessorsInfo, psiField.getName(), isBoolean);
            final Collection<PsiMethod> classMethods = PsiClassUtil.collectClassMethodsIntern(psiClass);
//            filterToleratedElements(classMethods);

            for (String methodName : methodNames) {
                if (PsiMethodUtil.hasSimilarMethod(classMethods, methodName, 0)) {
                    final String setterMethodName = Utils.getGetterName(psiField);

                    builder.addWarning(FlexerBundle.message("inspection.message.not.generated.s.method.with.similar.name.s.already.exists"),
    ...
    private boolean validateAccessorPrefix(@NotNull PsiField psiField, @NotNull ProblemBuilder builder) {
        ...
        if (AccessorsInfo.build(psiField).isPrefixUnDefinedOrNotStartsWith(psiField.getName())) {
            builder.addWarning(FlexerBundle.message("inspection.message.not.generating.getter.for.this.field"));
            result = false;
        }
        return result;
    }
    ...
    public PsiMethod createGetterMethod(@NotNull PsiField psiField, @NotNull PsiClass psiClass, @NotNull String methodModifier) {
        final String methodName = Utils.getGetterName(psiField);

        FlexerLightMethodBuilder methodBuilder = new FlexerLightMethodBuilder(psiField.getManager(), methodName)
        ...
        copyCopyableAnnotations(psiField, modifierList, Utils.BASE_COPYABLE_ANNOTATIONS);
    ...
    public FlexerPsiElementUsage checkFieldUsage(@NotNull PsiField psiField, @NotNull PsiAnnotation psiAnnotation) {
        return FlexerPsiElementUsage.READ;
    }
}
